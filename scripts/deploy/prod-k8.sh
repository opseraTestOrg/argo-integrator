...
docker run --rm --network host \
        ...
        aws --profile opsera-prod ecr get-login-password \
        --region us-east-2 \
        | docker login --username AWS \
        --password-stdin 440953937617.dkr.ecr.us-east-2.amazonaws.com

docker pull 440953937617.dkr.ecr.us-east-2.amazonaws.com/kubectl:laprod

docker run --rm --network host \
        -v ${PWD}/.tmp/kube-vol:/root/.kube \
        440953937617.dkr.ecr.us-east-2.amazonaws.com/kubectl \
        aws --profile opsera-prod eks update-kubeconfig --name ${clusername}

docker run --rm --name kubectl \
        -v ${PWD}/.tmp/kube-vol:/root/.kube \
        440953937617.dkr.ecr.us-east-2.amazonaws.com/kubectl:laprod \
